...
static void rmnet_vnd_update_rx_stats(struct net_device *net,
              unsigned int rx_packets, unsigned int rx_bytes) {
#if defined(MHI_NETDEV_STATUS64)
    struct qmap_priv *dev = netdev_priv(net);
    struct pcpu_sw_netstats *stats64 = this_cpu_ptr(dev->stats64);

    // 使用同步块来更新数据，确保多核环境下的原子性
...
#else
    // 对于非 64 位统计数据的情况，直接累加到 net_device 的统计字段
...
static void rmnet_vnd_update_tx_stats(struct net_device *net,
              unsigned int tx_packets, unsigned int tx_bytes) {
#if defined(MHI_NETDEV_STATUS64)
    struct qmap_priv *dev = netdev_priv(net);
    struct pcpu_sw_netstats *stats64 = this_cpu_ptr(dev->stats64);
...
static struct rtnl_link_stats64 *_rmnet_vnd_get_stats64(struct net_device *net, struct rtnl_link_stats64 *stats)
{
    ...
    // 将 net_device 的普通统计数据转换为 64 位统计数据
    netdev_stats_to_stats64(stats, &net->stats);

    // 如果存在 nss 回调，则重置 RX 统计数据
...
static int qmap_register_device(sQmiWwanQmap * pDev, u8 offset_id)
{
	...
	ether_addr_copy((u8 *)qmap_net->dev_addr, real_dev->dev_addr);
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
    ...
    if (possibly_iphdr(dev->net->dev_addr)) {
        u8 new_dev_addr[ETH_ALEN];

        // 复制现有的 dev_addr 到新数组中
        memcpy(new_dev_addr, dev->net->dev_addr, ETH_ALEN);

        // 修改 MAC 地址的第一个字节
        new_dev_addr[0] |= 0x02;  /* set local assignment bit */
        new_dev_addr[0] &= 0xbf;  /* clear "IP" bit */

        // 使用 eth_hw_addr_set 重新设置 dev_addr
        eth_hw_addr_set(dev->net, new_dev_addr);
    }
